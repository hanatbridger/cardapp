// mycollectrics.com card lookup + payload trimming. Shared by the
// user-facing /api/card-stats proxy and the daily cron's grading-alert
// sweep so both read the same PSA 10 price and population numbers.
//
// Pipeline:
...
//   2. GET /api/card/{id} → trim (~240KB) to dynamics + daily sold
//      aggregates + PSA 10 graded price series/population (~1KB).
...
/** Collectrics card id for a name + collector number; null when untracked. */
export async function resolveCollectricsId(
  name: string,
  number: string,
): Promise<number | null> {
...
/** Full upstream card payload. Throws on a non-2xx. */
export async function fetchCollectricsCard(id: number): Promise<any> {
  const res = await fetchWithTimeout(`${UPSTREAM}/card/${id}`, { headers: UA });
...
export function buildStats(card: any): StatsResponse {
...
export function buildPsa10(card: any): Psa10Stats | null {
...
/**
 * Trimmed stats for a name + number. null when collectrics doesn't
 * track the card; throws on an upstream failure so callers can tell
 * "untracked" from "unavailable".
 */
export async function fetchCollectricsStats(
  name: string,
  number: string,
): Promise<StatsResponse | null> {
  const id = await resolveCollectricsId(name, number);
  if (id === null) return null;
  return buildStats(await fetchCollectricsCard(id));
}
